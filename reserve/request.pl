            ...
            for my $item_object (@items) {
                ...
                if ($patron) {
                    ...
                    $item->{item_level_holds} =
                        Koha::CirculationRules->get_opacitemholds_policy( { item => $item_object, patron => $patron } );
                        ...
                        )
                    {
                        ...
                        if ( $item->{pickup_locations_count} > 0 || C4::Context->preference('AllowHoldPolicyOverride') )
                        {
                            ...
                            my $default_pickup_location =
                                $pickup_locations->search( { branchcode => $default_pickup_branch } )->next;
...
sub sort_borrowerlist {
    ...
    my $ref           = [];
    push @{$ref},
        sort { uc( $a->{surname} . $a->{firstname} ) cmp uc( $b->{surname} . $b->{firstname} ) } @{$borrowerslist};
    return $ref;
}
