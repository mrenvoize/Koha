...
# Copyright 2008 Garry Column and the Koha Development team
...
if ( $format =~ /(rss|atom|opensearchdescription)/ ) {
    ...
    || ( defined $cgi->param("q") && $cgi->param("q") ne "" )
    || ( $cgi->param('multibranchlimit') )
    || ( $cgi->param('limit-yr') )
    || @searchCategories )
{
...
my @sorted_itemtypes =
    sort { $itemtypes->{$a}->{translated_description} cmp $itemtypes->{$b}->{translated_description} } keys %$itemtypes;
foreach my $advanced_srch_type (@advanced_search_types) {
    ...
    if ( $advanced_srch_type eq 'itemtypes' ) {
        ...
        for my $thisitemtype (@$advsearchtypes) {
            ...
            next
                if $hidingrules->{$hiding_key} && any { $_ eq $thisitemtype->{authorised_value} }
...
my @sort_by;
my @allowed_sortby =
    qw /acqdate_asc acqdate_dsc author_az author_za call_number_asc call_number_dsc popularity_asc popularity_dsc pubdate_asc pubdate_dsc relevance title_az title_za/;
...
if ($tag) {
    ...
    eval {
        ( $error, $results_hashref, $facets ) = $searcher->search_compat(
            $query, $simple_query, \@sort_by, \@servers, $results_per_page, $offset, undef,
            $itemtypes_nocategory, $query_type, $scan, 1
        );
    };
}
    ...
    if ( $server && $server =~ /biblioserver/ ) {    # this is the local bibliographic server
        ...
        if ( $hits == 0 && $basic_search && $operands[0] !~ /^\".*\"$/ ) {
            ...
            eval {
                ( $error, $quoted_results_hashref, $facets ) = $searcher->search_compat(
                    $query, $simple_query,         \@sort_by,   \@servers, $results_per_page, $offset,
        ...
        foreach my $res (@newresults) {
            ...
            if (    C4::Context->preference('TagsEnabled')
                and C4::Context->preference('TagsShowOnList') )
            {
            ...
            $res->{artreqpossible} =
                ( $art_req_itypes->{ $res->{itemtype} // q{} } || $art_req_itypes->{'*'} ) ? 1 : q{};
        ...
        if ($hits) {
            ...
            my (
                $page_numbers,     $hits_to_paginate, $pages, $current_page_number, $previous_page_offset,
...
$template->{VARS}->{DidYouMean} =
    ( defined C4::Context->preference('OPACdidyoumean') && C4::Context->preference('OPACdidyoumean') =~ m/enable/ );
